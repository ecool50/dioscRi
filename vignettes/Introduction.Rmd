...
Since the training dataset has manual gating cell types, we can see how well our clustering matches the manual gates using the `Adjusted Rand Index` (ARI) score. We obtain a score of $0.812$ which shows good concordance.
```{r, eval=T}
aricode::ARI(sce_train$clusters_norm, sce_train$mg_cell_type_distinct)
```

# Annotate clustering
## The resulting clusters were expertly annotated using marker expressions so we can update the clustering
```{r, eval=T}
sce_train$clusters_norm <- recode_factor(sce_train$clusters_norm,
                                 "cluster_1" = "NK",
                                 "cluster_2" = "Not Naive CD8+ T cells", 
                                 "cluster_3" = "Naive CD8+ T cells",
                                 "cluster_4" = "NK/Mixed", 
                                 "cluster_5" = "Naive B Cells", 
                                 "cluster_6" = "CD4+ Tconv",
                                 "cluster_7" = "Memory B cells",
                                 "cluster_8" = "16+ monocytes",
                                 "cluster_9" = "CD141+ DC/pDCs",
                                 "cluster_10" = "14+ monocytes",
                                 "cluster_11" = "CD4+ Treg" 
                                 )
```
...
To assess how well the model is performing, we use the ROC-AUC curve. We observe an $AUC = 0.9$ for training and $AUC = 0.82$ for testing. 
...
Note that the testing AUC score shows an improvement over the previously reported score of $0.75$.

# Visualise and assess  model outputs
We can look at which features are driving my model's performance by looking at the feature tree which combines the hierarchy of the cell type proportions with marker means per cell type. For the hierarchy, the color of the node represents the value of the coefficient in the final model. For example, we see that there is a difference in proportions between CAD+ and CAD- for `Naive CD8+ T cells` with CAD- samples having a higher proportions (negative coefficients). Similar interpretation can be applied to the marker means per cell type. For example, both `FoxP3` and `CD194-CCR4` have positive coefficients in the `CD4+ Tregs` cell type indicating that these markers are expressed higher in `CD4+ Tregs` for CAD+ samples compared to CAD- samples. 
...
Using only the proportions as features, we get an $AUC = 0.77$ which is a decent score, but clearly underscores the benefits of combining the proportions with the marker means.
```{r}
...
## Visualise and assess  model outputs
Similar to the full model, we see similar proportions with non-zero coefficients. We also see a couple of new features including the combination of `NK` and `NK` mixed cells having a higher proportions in CAD+ samples. The overlapping group lasso is able to identify the relevance of the combination of these two cell types even though individually, the are not relevant to the model.
```{r, fig.height=10, fig.width=12}
visualiseModelTree(fit = fit$fit, tree = tree, type = "MG", 
...
```
Similar to the full model, only the `CD4+ Tregs` and the `Naive CD8+ T cells` have statistically significant proportions between CAD status.
...
## Predict on test data
Using only the marker means per cell type as features, we get an $AUC = 0.72$ which is less than just the proportion model but is still a decent score. Again, this clearly underscores the benefits of combining the proportions with the marker means.
```{r}
test_auc <- plotAUC(fit = fit$fit, xTest = X_test, yTest = y_test, 
...
## Visualise and assess  model outputs
Similar to the full model, we see similar marker means in cell types with non-zero coefficients. However, the magnitude of the coefficients have reduced (CD4+ Treg). This could imply that the cell type proportions provide more weighting to marker means per cell type.
```{r, fig.height=10, fig.width=12}
plotHeatmap(fit$fit, order = order, type = "MG")
```
...
plotSigFeatures(stats = stats, sigFeatures = sigFeatures, outcome = "Gensini bin", 
    ...
    xlab("Gensini bin")
```
As with the full model, none of the features have a statistically signicant difference between CAD status after adjusting for multiple testing. We do however see a reduction in overall magnitudes of various p-values.
