...
We apply two parallel asinh streams: one without derandomisation (`derand = FALSE`) for the data that goes through the MMD-VAE, and one with derandomisation (`derand = TRUE`) for selecting reference samples by covariance structure.
```{r}
...
df_test_scaled[, useMarkers] <- predict(preProcValues_raw, df_test_scaled[, useMarkers])
```

# Get the reference samples for training and validating the MMD-VAE model
To further speed up training of the MMD-VAE model, we select a set of $k$ samples which will be used for training. To select these samples, we first compute the covariance matrix for each sample; next the pairwise Frobenius norms of the covariances for all samples are computed; finally the top $k$ samples are selected by averaging all the Frobenius norms for each sample and returning the top $k$ with the minimum average. We set $k$ to $2$ in our analyses. Reference selection is performed on the derand=TRUE stream so that the per-sample covariances are computed on cleanly-jittered data.
...
val_dat   <- df_train[df_train$sample_id %in% res_ind$bottomNSamples, ]
```
...
After training the normalisation model, both the training and the testing data are normalised using the trained model. We pass the derand=FALSE streams (`df_train` and `df_test`) through the encoder/decoder.
```{r}
...
# Fit Overlap model
Finally, everything is ready for us to fit the prediction model. The model used is the overlapping group lasso that is described [here](https://pbreheny.github.io/grpreg/index.html). The model was originally designed for group based lasso. However, we have modified it to be applied to overlapping groups based on [here](https://pmc.ncbi.nlm.nih.gov/articles/PMC5026200/). To fit the model, we pass the features, response, and grouping structure. As with the standard lasso model, we jointly optimise $\alpha$ and $\lambda$ over a grid: $\alpha \in \{0.1, 0.2, \ldots, 1.0\}$ (step $0.1$) and $\lambda$ across $10{,}000$ values evenly spaced over $[0.05, 1]$. For every $(\alpha, \lambda)$ pair we compute $\text{BIC} = \text{deviance} + \text{df}\cdot\log n$; for each $\alpha$ the optimal $\lambda$ is selected via the elbow method on its BIC sequence, and the final $(\alpha, \lambda)$ is the combination that minimises BIC across all $\alpha$ values.
```{r}
# fit the model
...
```{r}
test_auc <- plotAUC(fit = fit$fit, xTest = X_test, yTest = y_test, 
                    title = "Test - Means AUC =")
test_auc$plot
```

## Visualise and assess  model outputs
```{r, fig.height=6, fig.width=8}
dioscRi:::plotHeatmap(fit$fit, order = order, type = "cluster")
```
...
# Clinical data
We can also add clincical data including Age and Sex. Below is an example where we add the Age and Sex clinical info to the full model

## For the training set
```{r}
encoded_gender_train <- model.matrix(~Gender-1, data=clinicaldata_train)
clinicaldata_train <- cbind(clinicaldata_train, encoded_gender_train)

Age <- factor(clinicaldata_train[match(row_names_train,clinicaldata_train$sample_id),
                                      "Age"])
GenderM <- factor(clinicaldata_train[match(row_names_train,clinicaldata_train$sample_id),
                                      c("GenderM")])

clinicaldata_info_train <- cbind(Age, GenderM)
```
...
Age_test <- factor(clinicaldata_test[match(row_names_test,clinicaldata_test$sample_id),
                                      "Age"])
GenderM_test <- factor(clinicaldata_test[match(row_names_test,clinicaldata_test$sample_id),
                                      c("GenderM")])

clinicaldata_info_test <- cbind(Age_test, GenderM_test)
```

# Add the clinical data to the
```{r, eval=T}
trees <- generateTree(features = prop_logit_train, method = "ward")
tree <- trees$tree
order <- trees$order
...
X_train <- predict(scaleVals, X_train) %>%
    as.matrix()
X_train <- cbind(X_train, clinicaldata_info_train)
```
...
X_test <- predict(scaleVals, X_test)
X_test <- cbind(X_test, clinicaldata_info_test)
```
...
# Manual gating
Next, we can perform the same experiment with the manually gated cell types. Since we already have the processed data, we can focus on
the manually gated cell types.

# Predict manual gating cell types
First we need to predict the manually gated cell types.
```{r}
# set up the training data
train_x <- reducedDim(sce_train, type = "VAE") %>%
...
# update the clusters for the test data
sce_test$clusters_mg <- df_test_clusters
```
...
## Proportions
```{r}
sce_train <- sce_train[, sce_train$mg_cell_type_distinct != "other"]
sce_train <- sce_train[, sce_train$mg_cell_type_distinct != ""]
...
data_train <- computeFeatures(sce = sce_train, featureType = 'prop', 
                              cellTypeCol = 'mg_cell_type_distinct', sampleCol = 'sample_id', 
                              logit = FALSE, useMarkers = useMarkers)
...
## Means
```{r, eval=T}
markerMeanCellType_train <- computeFeatures(sce = sce_train, featureType = 'mean', assay = "norm",
                              cellTypeCol = 'mg_cell_type_distinct', sampleCol = 'sample_id', 
                              logit = TRUE, useMarkers = useMarkers)
```


## Compute features on test
## Proportions
```{r}
sce_test <- sce_test[, sce_test$clusters_mg != "other"]
sce_test <- sce_test[, sce_test$clusters_mg != ""]
sce_test$clusters_mg <- droplevels(sce_test$clusters_mg)

data_logit_test <- computeFeatures(sce = sce_test, featureType = 'prop', 
                              cellTypeCol = 'clusters_mg', sampleCol = 'sample_id', 
                              logit = TRUE, useMarkers = useMarkers)
...
## Means
```{r, eval=T}
markerMeanCellType_test <- computeFeatures(sce = sce_test, featureType = 'mean',  assay = "norm",
                              cellTypeCol = 'clusters_mg', sampleCol = 'sample_id', 
                              logit = TRUE, useMarkers = useMarkers)
```

# Generate Tree
Here we generate a hierarchical tree for the manually gated data.
```{r, eval=T}
root <- Node$new("Root")
...
# Generate grouping structures
```{r}
nclust <- ncol(data_logit_train)
order <- as.phylo(root)$tip.label
tree <- findChildren(ggtree(as.phylo(root),
...
## Testing
```{r}
X_test<- data_logit_test %>%
  as.matrix()
X_test <- predict(scaleVals, X_test)
```

```{r}
groups <- generateGroups(tree = tree, nClust = nclust, proportions = data_logit_train,
...
## Predict on test data
```{r}
test_auc <- plotAUC(fit = fit$fit, xTest = X_test, yTest = y_test, title = "Test - Proportions AUC =")
test_auc$plot
```
...
```{r}
groups <- as.list(seq (1,ncol(markerMeanCellType_train),1)) 
```
...
## Predict on test data
```{r}
test_auc <- plotAUC(fit = fit$fit, xTest = X_test, yTest = y_test, title = "Test - Means AUC =")
test_auc$plot
```
...
order <- gsub("_", " ", order)
dioscRi:::plotHeatmap(fit$fit, type = "MG", order = order)
```
