    ...
    predict.grpregOverlap <- function(object, X,
      ...
      type <- match.arg(type)
      beta <- coef(
        object = object, lambda = lambda, latent = latent,
      ...
      if (length(dim(object$beta)) == 2) {
        ...
        if (type == "norm") {
          beta <- coef(
            object = object, lambda = lambda, latent = TRUE,
            which = which, drop = FALSE, ...
          )
          if (!latent) {
    ...
    coef.grpregOverlap <- function(object, lambda, latent = FALSE,
      ...
      ## latent beta
      beta <- coef(
        object = obj.new, lambda = lambda,
        which = which, drop = FALSE, ...
      )
      if (!latent) {
        beta <- gamma2beta(beta,
          incidence.mat = object$incidence.mat,
